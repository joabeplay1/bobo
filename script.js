...
let isGenerating=false, currentCode="", editMode=false, currentProjectId=null;
...
* Mobile First e Desktop otimizado
* UX profissional e animações suaves
...
REGRAS ESTABELECIDAS CONTRA ERROS 404 (OBRIGATÓRIO):
* NUNCA referencie arquivos externos de imagem, CSS ou JS (ex: src="app.js", href="style.css" ou src="./bobo/banner.jpg"). 
* Todo o CSS e JS deve estar 100% embutido e contido no único arquivo HTML.
* NUNCA use "via.placeholder.com". Se precisar de imagens de espaço reservado, use EXCLUSIVAMENTE a API placehold.co (Ex: https://placehold.co/800x400?text=Imagem).
...
function saveCurrentProject() {
  ...
  if (currentProjectId) {
    ...
    if (idx >= 0) { 
      ps[idx] = { ...ps[idx], code: currentCode, prompt: document.getElementById("prompt").value, updatedAt: Date.now() }; 
      saveProjects(ps); 
      renderHistory(); 
      return; 
    }
...
function loadProject(id) {
  ...
  showCode(p.code); 
  addLog("📂 Projeto carregado: " + p.name);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("message", e => {
    if (e.data?.type === "omega_text_edit" && e.data.html) {
      ...
      const co = document.getElementById("code-output");
      if(co) co.textContent = currentCode;
      saveCurrentProject();
    }
  });
});
...
function cleanCode(raw) {
  let code = raw.replace(/<think>[\s\S]*?<\/think>/gi, "")
                .replace(/```html\s*/gi, "")
...
async function robustFetch(url, options) {
  ...
  if (!response.ok) { 
    const msg = data?.error?.message || data?.error?.code || (typeof data?.error === "string" ? data.error : null) || data?.message || `HTTP ${response.status}: ${response.statusText}`; 
    throw new Error(msg); 
  }
...
async function callAPI(promptText) {
  ...
  // Declaração de objetos robusta. Tokens de saída configurados para limites seguros recomendados.
  const body = isGemini
    ? {
  ...
  // Removido AbortController para permitir geração ilimitada.
  const data = await robustFetch(url, { method: "POST", headers: headers, body: JSON.stringify(body) });
...
async function gerarProjeto() {
  ...
  try {
    ...
    if(!isGenerating) return; // Checa se o usuário parou manualmente a geração
    currentCode = code; 
...
async function runImprovement(improvPrompt) {
  ...
  try {
    ...
    if(!isGenerating) return;
    addLog("❌ Erro na melhoria: " + e.message);
  } finally {
    isGenerating = false; 
...
function injectEditor() {
  ...
  // FIX: Previne erro 5 - falha quando o corpo do documento do iframe ainda não existe
  if (!doc.body) return;

  let style = doc.getElementById("__omega_editor_style__");
  if (!style) { 
    style = doc.createElement("style"); 
    style.id = "__omega_editor_style__"; 
...
function removeEditor() {
  ...
  if (!iframe || !iframe.contentDocument) return;
  const doc = iframe.contentDocument;
  const style = doc.getElementById("__omega_editor_style__"); 
  if (style) style.remove();
  const els = doc.querySelectorAll("[data-omega-editable]");
  els.forEach(el => { 
    el.removeAttribute("contenteditable"); 
    el.removeAttribute("data-omega-editable"); 
